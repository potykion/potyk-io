...
from potyk_io_back.potyk_io.collections.movies import Movie, MovieCollection
...
@admin_bp.post("/cinema/movie")
@login_required
def cinema_add_movie():
    ...
    if year_error:
        ...
        return redirect(url_for("admin.cinema"))
...
@admin_bp.post("/cinema/collection")
@login_required
def cinema_create_collection():
...
@admin_bp.post("/cinema/collection/add-movie")
@login_required
def cinema_add_movie_to_collection():
...
@admin_bp.post("/cinema/collection/move-movie")
@login_required
def cinema_move_movie_between_collections():
...
@admin_bp.post("/cinema/movie/delete")
@login_required
def cinema_delete_movie():
    payload = request.get_json(silent=True) or {}
    movie_id = str(payload.get("movieId") or "").strip()
